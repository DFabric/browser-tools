...
InfoViewer.prototype = {
	...
	html_download_element_for_video: function (video) {
		...
		if (protocol === 'http') return this.html_video_link(url, 'Download media');
	...
	display_loading: function (url) {
		...
		progress.innerHTML = '<div class="loading" style="width: 100%;"></div>';
	...
	display: function () {
		...
		el.innerHTML = 'Medias for: ';
		el.appendChild(this.html_link(data.url));
		table = document.createElement('table');
		table.setAttribute('class', 'table');
		table.innerHTML = '<thead><tr><th>Thumbnail</th><th>Media title</th><th>Format</th><th>Download link</th></tr></thead>';
